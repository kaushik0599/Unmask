...
function AttackChainViz({ event, onReplay }: { event: UnmaskEvent | null; onReplay: () => void }) {
  ...
  const nodes = event
    ? [
        { id: "field", label: event.field_type ? `${event.field_type} field` : "Sensitive field", sub: event.metadata?.field_id ?? "unidentified", color: "#0ea5e9" },
        { id: "script", label: event.script_origin ?? "unknown", sub: `${event.vector ?? "unknown"} vector`, color: "#f59e0b" },
        { id: "dest", label: event.destination ?? "unknown", sub: "destination", color: blocked ? "#ef4444" : "#8a94a8" },
        { id: "fw", label: "UNMASK Firewall", sub: event.policy ?? "policy unavailable", color: blocked ? "#ef4444" : "#8a94a8" },
  ...
  return (
    <div className="card rounded-2xl p-6 mb-6">
      <div className="flex items-center justify-between mb-6">
        <div className="font-mono text-xs" style={{ color: "#44506a", letterSpacing: "0.1em" }}>ATTACK CHAIN</div>
      ...
      {!event ? (
        <div className="text-center py-10 font-mono text-xs" style={{ color: "#44506a" }}>
        ...
        <div className="relative">
          <svg className="absolute left-0 top-0 w-full h-full pointer-events-none" style={{ zIndex: 0 }} viewBox="0 0 100 100" preserveAspectRatio="none">
            {[20, 40, 60, 80].map((y, i) => (
              <line key={i} x1="50%" y1={`${y}%`} x2="50%" y2={`${y + 12}%`} stroke={i >= 2 && blocked ? "rgba(220,38,38,0.4)" : "rgba(14,165,233,0.3)"} strokeWidth="0.5" />
            ))}
              ...
              <div key={node.id} className="flex flex-col items-center">
                <div
                  ...
                  style={{
                    ...
                    background: i === 4 && blocked ? "rgba(220,38,38,0.06)" : "rgba(20,25,32,0.9)",
                ...
                >
                  ...
                  <span className="font-mono" style={{ fontSize: 10, color: "#44506a" }}>{node.sub}</span>
                </div>
                {i < nodes.length - 1 && (
                  <div className="flex flex-col items-center my-2">
                    <div style={{ width: 1, height: 20, background: i >= 2 && blocked ? "rgba(220,38,38,0.4)" : "rgba(14,165,233,0.3)" }} />
                    <svg width="10" height="10" viewBox="0 0 10 10">
                      <path d="M 2 2 L 5 8 L 8 2" stroke={i >= 2 && blocked ? "rgba(220,38,38,0.5)" : "rgba(14,165,233,0.4)"} strokeWidth="1.5" fill="none" />
                    </svg>
                  </div>
                )}
              </div>
...
function EvidencePanel({ event }: { event: UnmaskEvent | null }) {
  if (!event) {
    return (
      <div className="card rounded-xl p-6 mb-6">
        <div className="font-mono text-xs mb-5" style={{ color: "#44506a", letterSpacing: "0.1em" }}>EVIDENCE</div>
        <div className="font-mono text-xs" style={{ color: "#44506a" }}>No evidence recorded.</div>
      </div>
    );
  }
  ...
  const rows: Array<{ label: string; val: string; color: string }> = [
    { label: "FIELD TYPE", val: event.field_type ?? "unknown", color: "#0ea5e9" },
    { label: "SCRIPT ORIGIN", val: event.script_origin ?? "unknown", color: "#f59e0b" },
    { label: "DESTINATION", val: event.destination ?? "unknown", color: blocked ? "#ef4444" : "#c8bfb0" },
    { label: "VECTOR", val: event.vector ?? "unknown", color: "#c8bfb0" },
    { label: "POLICY", val: event.policy ?? "unavailable", color: "#c8bfb0" },
    { label: "ACTION", val: event.action ?? "unknown", color: blocked ? "#ef4444" : "#22c55e" },
    { label: "SEVERITY", val: event.severity.toUpperCase(), color: blocked ? "#ef4444" : "#8a94a8" },
    { label: "FINGERPRINT", val: shortHash(event.metadata?.field_hash), color: "#8b5cf6" },
    { label: "FIELD LENGTH", val: event.metadata?.field_length != null ? String(event.metadata.field_length) : "unavailable", color: "#44506a" },
    { label: "TIMESTAMP", val: clockTime(event.timestamp), color: "#44506a" },
  ];
      ...
      <div className="space-y-3">
        {rows.map((row) => (
          <div key={row.label} className="flex justify-between items-center py-2" style={{ borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
            <span className="font-mono text-xs" style={{ color: "#44506a", letterSpacing: "0.08em" }}>{row.label}</span>
...
function Timeline({ events }: { events: UnmaskEvent[] }) {
  ...
  return (
    <div className="card rounded-xl p-6 mb-6">
      <div className="font-mono text-xs mb-5" style={{ color: "#44506a", letterSpacing: "0.1em" }}>FORENSIC TIMELINE</div>
      <div>
        {events.map((event, i) => {
            ...
            <div key={event.event_id} className="timeline-item">
              <div className="flex-shrink-0 mt-0.5">
                <div
                  style={{
                    ...
                    background: type === "blocked" ? "rgba(220,38,38,0.15)" : type === "incident" ? "rgba(14,165,233,0.1)" : "rgba(255,255,255,0.04)",
                    border: `1.5px solid ${type === "blocked" ? "#ef4444" : type === "incident" ? "#0ea5e9" : "rgba(255,255,255,0.1)"}`,
              ...
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-0.5 flex-wrap">
                  <span className="font-mono text-xs" style={{ color: "#44506a" }}>{clockTime(event.timestamp)}</span>
                  <span style={{ fontSize: 13, fontWeight: 500, color: type === "blocked" ? "#ef4444" : "#e8e2d8" }}>
                    {event.event_type} · {event.action}
                  </span>
                </div>
                <span className="font-mono" style={{ fontSize: 11, color: "#44506a" }}>
...
function WhyBlocked({ incidentId, primaryAction }: { incidentId: string; primaryAction: string | null }) {
  ...
  return (
    <div className="card rounded-xl p-6 mb-6" style={{ borderColor: "rgba(14,165,233,0.15)" }}>
      <div className="font-mono text-xs mb-4" style={{ color: "#44506a", letterSpacing: "0.1em" }}>{title}</div>

      {state === "loading" && <p className="font-mono text-xs" style={{ color: "#44506a" }}>Generating evidence-based explanation…</p>}
      ...
      {state === "ready" && explanation && (
        <>
          <p style={{ fontSize: 14, color: "#c8bfb0", lineHeight: 1.85 }}>{explanation.verdict}</p>
          {explanation.findings.length > 0 && (
            <div className="mt-4 space-y-2">
              {explanation.findings.map((f) => (
                <div key={f.event_id} className="font-mono" style={{ fontSize: 11, color: "#8a94a8" }}>
                  [{f.event_index}] {f.statement}
                </div>
              ))}
            </div>
          )}
          <div className="mt-4 px-4 py-3 rounded-lg" style={{ background: "rgba(14,165,233,0.05)", border: "1px solid rgba(14,165,233,0.12)" }}>
            <span className="font-mono text-xs" style={{ color: "#0ea5e9" }}>
...
export default function IncidentDetailPage() {
  ...
  if (state === "loading") {
    return <ConsoleShell><div className="p-12 font-mono text-xs" style={{ color: "#44506a" }}>Loading incident…</div></ConsoleShell>;
  }
  if (state === "unavailable") {
    return <ConsoleShell><div className="p-12 font-mono text-xs" style={{ color: "#ef4444" }}>Backend unavailable.</div></ConsoleShell>;
  }
  if (state === "notfound") {
    return <ConsoleShell><div className="p-12 font-mono text-xs" style={{ color: "#44506a" }}>Incident not found.</div></ConsoleShell>;
  }
    ...
    <ConsoleShell>
      <div style={{ maxWidth: 980, margin: "0 auto", padding: "48px 32px" }}>
        <div className="flex items-center gap-3 mb-8">
          <button onClick={() => navigate("/console")} style={{ color: "#44506a", background: "none", border: "none", cursor: "pointer", fontSize: 13 }}>
            ← Overview
          </button>
          <span style={{ color: "#44506a" }}>/</span>
          <button style={{ fontSize: 13, color: "#44506a", background: "none", border: "none", cursor: "pointer" }} onClick={() => navigate("/console/incidents")}>
            Incidents
          </button>
          <span style={{ color: "#44506a" }}>/</span>
          <span style={{ fontSize: 13, color: "#e8e2d8" }}>{incident.id}</span>
        </div>
        ...
        <div className="flex items-start justify-between mb-8 flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-3 mb-3 flex-wrap">
              ...
              <span className="font-mono text-xs" style={{ color: "#44506a" }}>{incident.id}</span>
            </div>
            <h1 className="font-bold mb-2" style={{ fontSize: 24, color: "#e8e2d8", letterSpacing: "-0.02em" }}>
              {incident.title}
            </h1>
            <div className="flex items-center gap-4 flex-wrap">
              <span className="font-mono text-xs" style={{ color: "#44506a" }}>{incident.website}</span>
              <span style={{ fontSize: 12, color: "#44506a" }}>{relativeTime(incident.updated_at)}</span>
            </div>
          </div>
        ...
        <div className="card rounded-xl p-4 mb-6 flex items-center gap-4 flex-wrap">
          <div className="font-mono text-xs" style={{ color: "#44506a", letterSpacing: "0.08em" }}>SITE STATE</div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <div className="card rounded-xl p-5">
              <div className="font-mono text-xs mb-4" style={{ color: "#44506a", letterSpacing: "0.1em" }}>ACTIONS</div>
